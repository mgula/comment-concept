...
function toggleinfo(e) {
	...
	if ($('#dislikebtn').hasClass('sent')) {
		$('#dislikebtn').removeClass('sent');
	};
	if ($('#likebtn').hasClass('clicked')) {
		$('#likebtn').removeClass('clicked');
	};
	if ($('#likebtn').hasClass('sent')) {
		$('#likebtn').removeClass('sent');
	};
...
/*To do:
	- resize infobox_ after comment submission
	- certain feature names get cut off, e.g. Road A/SR 7 (Road, Bridge, & Mall Connector Study) --> Road A/SR 7 (Road, Bridge, 
*/

$(".Like").click(function () {
	if (!($('#likebtn').hasClass('sent') || $('#dislikebtn').hasClass('sent'))) {
		if (!$('#likebtn').hasClass('clicked')) {
			if ($('#dislikebtn').hasClass('clicked')) {
				$('#dislikebtn').removeClass('clicked');
			};
...
$(document).on('submit','#ajax-contact',function(e) {
	...
	if ($('#likebtn').hasClass('clicked')) {
		vote = "like";
	} else if ($('#dislikebtn').hasClass('clicked')) {
		vote = "dislike";
	} else {
		vote = "none";
	};
	
	data = data + "&feature=" + $('#projectName').text() + "&vote=" + vote;
	testVar = data; //delete later
	$.ajax({
		type: 'POST',
		...
		if ($('#likebtn').hasClass('clicked')) {
			$('#likebtn').removeClass('clicked');
			$('#likebtn').addClass('sent');
		};
		if ($('#dislikebtn').hasClass('clicked')) {
			$('#dislikebtn').removeClass('clicked');
			$('#dislikebtn').addClass('sent');
		};
	}).fail(function(data) {
		/*delete this later*/
		if ($('#deniedstrings').text() == ' ') {
			$('#deniedstrings').text(testVar); 
		} else {
			$('#deniedstrings').append("\n" + testVar);
		}
		/**/
		if (data.responseText !== '') {
			$('#formDiv').text(data.responseText);
		} else {
			$('#formDiv').text('Oops! An error occured and your message could not be sent.');
		}
	});
});

/*delete this later*/ 
$(document).on('submit','#getComs',function(e) {
	e.preventDefault();
	$.ajax({
		type: 'GET',
		url: $('#getComs').attr('action')
	}).done(function(response) {
		$('#testComments').text(response);
		$('#DBcleared').text('');
	}).fail(function(data) {
		if (data.responseText !== '') {
			$('#testComments').text(data.responseText);
		} else {
			$('#testComments').text('Oops! Something went wrong.');
		}
	});
});
$(document).on('submit','#clearDB',function(e) {
	e.preventDefault();
	$.ajax({
		type: 'GET',
		url: $('#clearDB').attr('action')
	}).done(function(response) {
		$('#DBcleared').text(response);
	}).fail(function(data) {
		if (data.responseText !== '') {
			$('#DBcleared').text(data.responseText);
		} else {
			$('#DBcleared').text('Oops! Something went wrong.');
		}
	});
});
$(document).on('submit','#csv',function(e) {
	...
	$.ajax({
		type: 'GET',
		url: $('#csv').attr('action')
	}).done(function(response) {}).fail(function(data) {});
});
/**/
