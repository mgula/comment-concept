...
//issues/thoughts:
// - buttons and textarea will need to be set back to unclicked and empty whenever infobox_ loses 
//   active class or a new feature is clicked (or, buttons need to maintain state for all features).
// - actual vote will probably need to be sent to db ONLY when submit button is clicked.
// - should be able to send multiple comments per session.
...
$('.SubmitButton').click(function () {
	//this check will go in the .php file
	if ($('#dislikebtn').hasClass('clicked')) {
        //send dislike and comment to db
    } else if ($('#likebtn').hasClass('clicked')) {
        //send like and comment to db
    } else {
		//send comment to db
	};
	
	
	//the lines below are never reached for some reason but the idea is to hide the textarea and 
	//input elements, and make the labelfield (temporary success message) visible. note: not using 
	//a form at the moment because the forward slash in action=/whatever.php messes things up in actions.js
	$('#commentForm').css("display", "none");
	$('#tempButtonDiv').css("display", "none");
	$('#submitFeedback').css("display", "inline");
});
